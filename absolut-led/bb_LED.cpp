    ...
    switch (_state) {
      case FADE_UP:
        if (_counter <= 0) {
          ...
          _brightness = slope[SLOPE_SIZE - _counter] * _brightnessScale;
          _counter--;
          _resetTimer();
        }
      ...
      case FADE_DOWN:
        if (_counter <= 0) {
          ...
          _brightness = slope[_counter - 1] * _brightnessScale;
          _counter--;
          _resetTimer();
        }
...
void bb_LED::setMaxBrightness(int val){
    _maxBrightness = val;
    _brightnessScale = 255 /  val;
